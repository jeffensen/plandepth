...
class BackInductionDiscountHyperbolicThetaLowProbPruningkmax30(object):
    def __init__(self,
    ...
    def compute_state_values(self, block):
        ...
        # self.tp_mean0[:,self.noise[0]][:,self.noise[0]]
        prob = self.tp_mean1[:,self.noise[0]]#[:,self.noise[0]]
        odds_against_success = (1 - prob)/prob
        # torch.einsum('...ij, ...ij ->...ij', self.k, gamma) 
        #print()
        #print("odds_against_success.shape = ", odds_against_success.shape)
        #print("odds_against_success = ", odds_against_success)        
        #print("odds_against_success[:,self.noise[0]].shape = ", odds_against_success[:,self.noise[0]].shape)        
        #print("self.noise[0] = ", self.noise[0])
        ...
        if len(odds_against_success.shape) == 2:
            ...
            #print("self.k.shape = ", self.k.shape)
            #gamma = 1.0 / (1 + torch.einsum('...i, ...ij ->...ij', odds_against_success[:], self.k))            
