...
class BackInductionNoLearning(object):
    def __init__(self,
    ...
    def set_parameters(self, trans_par=None, true_params=False):
        ...
        self.tp_mean0 = torch.tensor([.9]).expand(self.batch_shape + (2,))
    ...
    def compute_state_values(self, block):
        ...
        for d in range(1, depth+1):
            ...
            # set state value
            Vs.append(p * Q[..., 1] + (1-p) * Q[..., 0]) # Note (LG): Values are calculated based on the current policy!!! (not based on random transitions)
            # Note (LG): An alternative would be to replace "p" either by "0.5", equivalent to calculating values based on a random policy,
            # or by the max of Q(jump) and Q(move)
