...
class BackInductionDiscountNoiseThetaAnchorPruningHiLoBeta(object):
    def __init__(self,
    ...
    def set_parameters(self, trans_par=None, true_params=False):
        ...
        if trans_par is not None:
            if true_params:
                self.beta_loNoise = (trans_par[..., 0]) # Response noise
                self.beta_hiNoise = (trans_par[..., 1]) # Response noise                
                self.theta = trans_par[..., 2] # Bias term in sigmoid function fot action-selection
                #self.alpha = (trans_par[..., 2]) # Learning rate for belief update     
                self.gamma = (trans_par[..., 3]) # Discount factor for Q values in low + high noise                                
            else:
                ...
                self.beta_loNoise = (trans_par[..., 0]).exp() # Response noise
                self.beta_hiNoise = (trans_par[..., 1]).exp() # Response noise                                
                self.theta = trans_par[..., 2] # Bias term in sigmoid function fot action-selection
                #self.alpha = (trans_par[..., 2]).sigmoid() # Learning rate for belief update                
                self.gamma = (trans_par[..., 3]).sigmoid() # Discount factor for Q values in low+high noise
        else:
            self.beta_loNoise = torch.tensor([10.]).repeat(self.runs)
            self.beta_hiNoise = torch.tensor([10.]).repeat(self.runs)            
            self.theta = zeros(self.runs)
            #self.alpha = zeros(self.runs)
            self.gamma = zeros(self.runs)             

        self.batch_shape = self.beta_loNoise.shape
    ...
    def plan_actions(self, block, trial):
        ...
        beta_loNoise = self.beta_loNoise[..., None]
        beta_hiNoise = self.beta_hiNoise[..., None]        
        theta = self.theta[..., None]
        
        D_times_beta = D * ((self.noise[0]==1) * beta_hiNoise + (self.noise[0]==0) * beta_loNoise)
        #self.logits.append(D * beta + theta)
        self.logits.append(D_times_beta + theta)        
