...
def load_and_format_behavioural_data(local_path, filenames): #kann es nicht als komplette funktion, nur wenn ich alles einzeln abspiele und die paths per Hand ergänze
    ...
    return stimuli, mask, responses, conditions, ids, subj_IDs
...
from agents_nonlearning import BackInductionNoLearning
...
def variational_inference(stimuli, mask, responses):
    ...
    # define agent
    agent = BackInductionNoLearning(confs,
                          runs=runs,
    ...
    infer.fit(num_iterations=3) # 1000
...
localpath = 'H:/Sesyn/TRR265-B09/Analysis SAT-PD2 Sophia/SAT_PD_Inference_Scripts' # LG

filenames = ["space_adventure_pd-results.json",
             "space_adventure_pd_inv-results.json"]    # posible filenames of SAT logfiles

stimuli_oa, mask_oa, responses_oa, conditions_oa, ids_oa, subj_IDs_oa = load_and_format_behavioural_data(path1, filenames)
stimuli_ya, mask_ya, responses_ya, conditions_ya, ids_ya, subj_IDs_ya = load_and_format_behavioural_data(path2, filenames)
...
def format_posterior_samples(infer):
    ...
    pars_df.drop([r'$\tilde{\beta}$'], axis=1, inplace=True) # , r'$\tilde{\alpha}$'
...
pars_df_ya['subjIDs'] = np.array(subj_IDs_ya)[pars_df_ya.subject.values - 1]
...
axes.plot(infer_oa.loss[100:])  
axes.set_title('ELBO Testdaten')
fig.savefig('ELBO Testdaten_oa_nolearning_0cost.jpg')


fig, axes = plt.subplots(1, 1, figsize=(15, 5))
axes.plot(infer_ya.loss[100:])  
axes.set_title('ELBO Testdaten')
fig.savefig('ELBO Testdaten_ya_nolearning_0cost.jpg')
...
g.fig.savefig('parameter_participant_nolearning_oa_0cost.jpg')
...
g.fig.savefig('parameter_participant_nolearning_ya_0cost.jpg')
...
g.fig.savefig('post_group_parameters_nolearning_OA_YA_0cost.pdf', dpi=300)
...
pars_df.to_csv('pars_post_samples_nolearning_0cost.csv')
...
np.savez('oa_plandepth_stats_B03_nonlearning_0cost', post_depth_oa, m_prob_oa, exc_count_oa)

post_depth_ya, m_prob_ya, exc_count_ya = get_posterior_stats(post_marg_ya)
np.savez('ya_plandepth_stats_B03_nonlearning_0cost', post_depth_ya, m_prob_ya, exc_count_ya)



#file = b = np.load('/home/sass/Dokumente/plandepth/oa_plandepth_stats_B03.npz', allow_pickle=True)
file = b = np.load(localpath + '/oa_plandepth_stats_B03_nonlearning_0cost.npz', allow_pickle=True)
...
pd.DataFrame(post_meanPD_firstAction_oa).to_csv(localpath + '/meanPD_1st_action_oa_single_nonlearning_0cost.csv')
file.close()



#file = b = np.load('/home/sass/Dokumente/plandepth/ya_plandepth_stats_B03.npz', allow_pickle=True)
file = b = np.load(localpath + '/ya_plandepth_stats_B03_nonlearning_0cost.npz', allow_pickle=True)
...
pd.DataFrame(post_meanPD_firstAction_ya).to_csv(localpath + '/meanPD_1st_action_oa_single_nonlearning_0cost.csv')
file.close()
...
m_param_count = 2 # beta and theta
...
pd.DataFrame(infer_ya.loss).to_csv(localpath + '/ELBO_ya_group_nonlearning_0cost.csv') #     
pd.DataFrame(infer_oa.loss).to_csv(localpath + '/ELBO_oa_group_nonlearning_0cost.csv') # 
...
dict_nll_oa['subjIDs'] = subj_IDs_oa
df_nll_oa = pd.DataFrame(data=dict_nll_oa)
df_nll_oa.to_csv('NLL_oa_group_nonlearning_0cost.csv') # Can be moved into the loop
...
dict_nll_ya['subjIDs'] = subj_IDs_ya
df_nll_ya = pd.DataFrame(data=dict_nll_ya)
df_nll_ya.to_csv('NLL_ya_group_nonlearning_0cost.csv') # Can be moved into the loop
...
import pandas as pd
pd.DataFrame(exc_count_oa_PD1).to_csv(localpath+"/exc_PD1_oa.csv")
file.close()

exc_count_ya_PD1 = exc_count_ya_1[0] [:] [:]
pd.DataFrame(exc_count_ya_PD1).to_csv(localpath+"/exc_PD1_ya.csv")
file.close()

# PD 2
exc_count_oa_PD2 = exc_count_oa_1[1] [:] [:]
pd.DataFrame(exc_count_oa_PD2).to_csv(localpath+"/exc_PD2_oa.csv")
file.close()

exc_count_ya_PD2 = exc_count_ya_1[1] [:] [:]
pd.DataFrame(exc_count_ya_PD2).to_csv(localpath+"/exc_PD2_ya.csv")
file.close()

# PD 3
exc_count_oa_PD3 = exc_count_oa_1[2] [:] [:]
import pandas as pd
pd.DataFrame(exc_count_oa_PD3).to_csv(localpath+"/exc_PD3_oa.csv")
file.close()

exc_count_ya_PD3 = exc_count_ya_1[2] [:] [:]
import pandas as pd
pd.DataFrame(exc_count_ya_PD3).to_csv(localpath+"/exc_PD3_ya.csv")
file.close()
