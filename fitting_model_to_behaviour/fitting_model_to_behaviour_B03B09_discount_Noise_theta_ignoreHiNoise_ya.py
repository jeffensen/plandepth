...
g.fig.savefig(localpath + '/post_group_parameters_YA_discount_Noise_theta_0cost_ignoreHinoise.pdf', dpi=300)
...
for ns, ax in zip(range(nplots), axes):
    sns.heatmap(m_prob_ya[1][:, ns], cmap='viridis', ax=ax, cbar=True)
...
for ns, ax in zip(range(nplots), axes):
    sns.heatmap(exc_count_ya[1][..., ns].T/n_samples, cmap='viridis', ax=ax, cbar=True)
    ax.set_xticklabels([1, 2, 3])
    ax.set_xlabel('depth')
