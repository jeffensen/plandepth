    ...
    def set_parameters(self, trans_par=None, true_params=False):
        ...
        if trans_par is not None:
            if true_params:
                ...
                self.prob = (trans_par[..., 2]) # Learning rate for belief update     
                ...
                self.theta = trans_par[..., 1] # Bias term in sigmoid function fot action-selection
                self.prob = (trans_par[..., 2]).sigmoid() # Learning rate for belief update                
        ...
        self.tp_mean0 = torch.tensor([self.prob, self.prob]).expand(self.batch_shape + (2,))
