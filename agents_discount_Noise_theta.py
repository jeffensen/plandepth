...
class BackInductionDiscountNoiseTheta(object):
    def __init__(self,
    ...
    def set_parameters(self, trans_par=None, true_params=False):
        ...
        if trans_par is not None:
            if true_params:
                ...
                self.gamma = (trans_par[..., 2]) # Discount factor for Q values in low + high noise                                
            else:
                ...
                self.gamma = (trans_par[..., 2]).sigmoid() # Discount factor for Q values in low+high noise                
