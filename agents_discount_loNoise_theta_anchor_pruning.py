...
class BackInductionDiscountLoNoiseThetaAnchorPruning(object):
    def __init__(self,
    ...
    def set_parameters(self, trans_par=None, true_params=False):
        ...
        if trans_par is not None:
            if true_params:
                ...
                #self.gamma_hiNoise = (trans_par[..., 2]) # Discount factor for Q values in high noise
            else:
                ...
                #self.gamma_hiNoise = (trans_par[..., 2]).sigmoid() # Discount factor for Q values in high noise
    ...
    def compute_state_values(self, block):
        ...
        for d in range(1, depth+1):
            ...
            #Qjump = torch.einsum('...i, ...ij ->...ij', (self.noise[0]==1) * self.gamma_hiNoise + (self.noise[0]==0) * torch.tensor(1.0),  Q[...,1])            
            Qjump = torch.einsum('...i, ...ij ->...ij', (self.noise[0]==1) * torch.tensor(1.0) + (self.noise[0]==0) * self.gamma_loNoise,  Q[...,1]) # Corrected            
