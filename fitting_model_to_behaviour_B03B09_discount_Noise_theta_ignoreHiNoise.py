...
localpath = 'H:/Sesyn/TRR265-B09/Analysis_SAT-PD2_Sophia/SAT_PD_Inference_Scripts/Results_discount_Noise_theta_ignoreHinoise' # LG # 
...
print('')
print('Inference assumes that high noise is ignored!')
conditions_oa_orig = conditions_oa.numpy().copy() # Will be needed for calculating fit via NLL
conditions_ya_orig = conditions_ya.numpy().copy()

conditions_oa[0] = 0*conditions_oa[0]
conditions_ya[0] = 0*conditions_ya[0]
...
fig.savefig(localpath + '/ELBO Testdaten_oa_discount_Noise_theta_0cost_ignoreHinoise.jpg')
...
g.fig.savefig(localpath + '/parameter_participant_oa_discount_Noise_theta_0cost_ignoreHinoise.jpg')
...
axes.plot(range(len(df_loss[0].rolling(window=25).mean())-100), df_loss[0].rolling(window=25).mean()[100:], lw=1)    #                 
axes.set_title('ELBO Testdaten')
fig.savefig(localpath + '/ELBO Testdaten_ya_discount_Noise_theta_0cost_ignoreHinoise.jpg')
...
g.fig.savefig(localpath + '/parameter_participant_ya_discount_Noise_theta_0cost_ignoreHinoise.jpg')
...
g.fig.savefig(localpath + '/post_group_parameters_OA_YA_discount_Noise_theta_0cost_ignoreHinoise.pdf', dpi=300)
...
pars_df.to_csv(localpath + '/pars_post_samples_discount_Noise_theta_0cost_ignoreHinoise.csv')
...
np.savez(localpath + '/oa_plandepth_stats_B03_discount_Noise_theta_0cost_ignoreHinoise', post_depth_oa, m_prob_oa, exc_count_oa)

post_depth_ya, m_prob_ya, exc_count_ya = get_posterior_stats(post_marg_ya)
np.savez(localpath + '/ya_plandepth_stats_B03_discount_Noise_theta_0cost_ignoreHinoise', post_depth_ya, m_prob_ya, exc_count_ya)



#file = b = np.load('/home/sass/Dokumente/plandepth/oa_plandepth_stats_B03.npz', allow_pickle=True)
file = b = np.load(localpath + '/oa_plandepth_stats_B03_discount_Noise_theta_0cost_ignoreHinoise.npz', allow_pickle=True)
...
pd.DataFrame(df_oa_meanPD).to_csv(localpath + '/meanPD_1st_action_oa_single_discount_Noise_theta_0cost_ignoreHinoise.csv')
file.close()



#file = b = np.load('/home/sass/Dokumente/plandepth/ya_plandepth_stats_B03.npz', allow_pickle=True)
file = b = np.load(localpath + '/ya_plandepth_stats_B03_discount_Noise_theta_0cost_ignoreHinoise.npz', allow_pickle=True)
...
pd.DataFrame(df_ya_meanPD).to_csv(localpath + '/meanPD_1st_action_ya_single_discount_Noise_theta_0cost_ignoreHinoise.csv')
dict_ids_ya={}
...
for i_mblk in range(140):
    for i_action in range(1): # Consider only first action; use range(3) to consider all action steps
        ...
        for i_subj in range(n_subj_oa):
            ...
            nll_hinoise_all_oa[i_subj, :] = np.matmul(nll_firstaction_oa_depths[i_subj, :, :].transpose(), conditions_oa_orig[0][i_subj, :]) # .numpy()# Sum of NLL for high noise (noise==1) 
            nll_lonoise_all_oa[i_subj, :] = np.matmul(nll_firstaction_oa_depths[i_subj, :, :].transpose(), 1 - conditions_oa_orig[0][i_subj, :]) # .numpy() # Sum of NLL for low noise (noise==0)
            nll_all_oa[i_subj, :] = nll_hinoise_all_oa[i_subj, :] + nll_lonoise_all_oa[i_subj, :]
            nll_hinoise_120_oa[i_subj, :] = np.matmul(nll_firstaction_oa_depths[i_subj, 20:, :].transpose(), conditions_oa_orig[0][i_subj, 20:]) # .numpy() # Exclude 20 training miniblocks
            nll_lonoise_120_oa[i_subj, :] = np.matmul(nll_firstaction_oa_depths[i_subj, 20:, :].transpose(), 1 - conditions_oa_orig[0][i_subj, 20:]) #.numpy()) 
            nll_120_oa[i_subj, :] = nll_hinoise_120_oa[i_subj, :] + nll_lonoise_120_oa[i_subj, :]

            nll_hinoise_120_mean_oa[i_subj] = np.matmul(nll_firstaction_oa_mean[i_subj, 20:].transpose(), conditions_oa_orig[0][i_subj, 20:]) #.numpy()) # Exclude 20 training miniblocks
            nll_lonoise_120_mean_oa[i_subj] = np.matmul(nll_firstaction_oa_mean[i_subj, 20:].transpose(), 1 - conditions_oa_orig[0][i_subj, 20:]) #.numpy()) 
...
for i_mblk in range(140):
    for i_action in range(1): # Consider only first action; use range(3) to consider all action steps
        ...
        for i_subj in range(n_subj_ya):
            ...
            nll_hinoise_all_ya[i_subj, :] = np.matmul(nll_firstaction_ya_depths[i_subj, :, :].transpose(), conditions_ya_orig[0][i_subj, :]) #.numpy()) # Sum of NLL for high noise (noise==1) 
            nll_lonoise_all_ya[i_subj, :] = np.matmul(nll_firstaction_ya_depths[i_subj, :, :].transpose(), 1 - conditions_ya_orig[0][i_subj, :]) #.numpy()) # Sum of NLL for low noise (noise==0)
            nll_all_ya[i_subj, :] = nll_hinoise_all_ya[i_subj, :] + nll_lonoise_all_ya[i_subj, :]
            nll_hinoise_120_ya[i_subj, :] = np.matmul(nll_firstaction_ya_depths[i_subj, 20:, :].transpose(), conditions_ya_orig[0][i_subj, 20:]) #.numpy()) # Exclude 20 training miniblocks
            nll_lonoise_120_ya[i_subj, :] = np.matmul(nll_firstaction_ya_depths[i_subj, 20:, :].transpose(), 1 - conditions_ya_orig[0][i_subj, 20:]) #.numpy()) 
            nll_120_ya[i_subj, :] = nll_hinoise_120_ya[i_subj, :] + nll_lonoise_120_ya[i_subj, :]

            nll_hinoise_120_mean_ya[i_subj] = np.matmul(nll_firstaction_ya_mean[i_subj, 20:].transpose(), conditions_ya_orig[0][i_subj, 20:]) #.numpy()) # Exclude 20 training miniblocks
            nll_lonoise_120_mean_ya[i_subj] = np.matmul(nll_firstaction_ya_mean[i_subj, 20:].transpose(), 1 - conditions_ya_orig[0][i_subj, 20:]) #.numpy()) 
...
pd.DataFrame(infer_ya.loss).to_csv(localpath + '/ELBO_ya_group_discount_Noise_theta_0cost_ignoreHinoise.csv') #     
pd.DataFrame(infer_oa.loss).to_csv(localpath + '/ELBO_oa_group_discount_Noise_theta_0cost_ignoreHinoise.csv') # 
...
df_nll_oa.to_csv(localpath + '/NLL_oa_group_discount_Noise_theta_0cost_ignoreHinoise.csv') 
...
df_nll_ya.to_csv(localpath + '/NLL_ya_group_discount_Noise_theta_0cost_ignoreHinoise.csv') 
...
import pandas as pd
pd.DataFrame(exc_count_oa_PD1).to_csv(localpath+"/exc_PD1_oa_discount_Noise_theta_0cost_ignoreHinoise.csv")
file.close()

exc_count_ya_PD1 = exc_count_ya_1[0] [:] [:]
pd.DataFrame(exc_count_ya_PD1).to_csv(localpath+"/exc_PD1_ya_discount_Noise_theta_0cost_ignoreHinoise.csv")
file.close()

# PD 2
exc_count_oa_PD2 = exc_count_oa_1[1] [:] [:]
pd.DataFrame(exc_count_oa_PD2).to_csv(localpath+"/exc_PD2_oa_discount_Noise_theta_0cost_ignoreHinoise.csv")
file.close()

exc_count_ya_PD2 = exc_count_ya_1[1] [:] [:]
pd.DataFrame(exc_count_ya_PD2).to_csv(localpath+"/exc_PD2_ya_discount_Noise_theta_0cost_ignoreHinoise.csv")
file.close()

# PD 3
exc_count_oa_PD3 = exc_count_oa_1[2] [:] [:]
import pandas as pd
pd.DataFrame(exc_count_oa_PD3).to_csv(localpath+"/exc_PD3_oa_discount_Noise_theta_0cost_ignoreHinoise.csv")
file.close()

exc_count_ya_PD3 = exc_count_ya_1[2] [:] [:]
import pandas as pd
pd.DataFrame(exc_count_ya_PD3).to_csv(localpath+"/exc_PD3_ya_discount_Noise_theta_0cost_ignoreHinoise.csv")
file.close()
